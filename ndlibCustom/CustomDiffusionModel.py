...
class CustomDiffusionModel(object):
    """
        Partial Abstract Class that defines Custom Diffusion Models
    """
    ...
    def __validate_configuration(self, configuration):
        ...
        """
        if "Infected_A" not in self.available_statuses:
            raise ConfigurationException("'Infected_A' status not defined.")
        if "Infected_S" not in self.available_statuses:
            raise ConfigurationException("'Infected_S' status not defined.")
    ...
    def set_initial_status(self, configuration):
        ...
        if 'Infected_A' not in self.params['status'] or 'Infected_S' not in self.params['status']:
